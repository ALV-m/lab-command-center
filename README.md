...
- **Check-ins** — a mandatory full-screen login form appears when a session starts and after every admin lock, with user-type buttons for **Student**, **Teacher**, **Visitor**, and **Administrator**. Students enter name, phone, admission number, course, class, and reason (optional email + photo); teachers and visitors enter name, phone, a reason, and optionally an ID/email/photo. Choosing **Administrator** locks the PC and shows the Windows sign-in screen, where the administrator logs into their own Windows account (configured on the Agent page); the agent records that sign-in automatically and never shows the form to the admin account. Submissions are recorded on the Check-ins page with a Student/Teacher/Visitor/Admin role badge. Choose the lab's sign-in method on the Agent page: per-student Windows passwords plus the form, or "login form instead of password" where every PC boots into one local account straight to the form (no Windows password page) — the server auto-generates a random non-admin account if you don't supply one, and the agent applies the Windows Hello fix so auto-login works on password-protected PCs. The Agent page can also block downloads and installs on the shared account (Software Restriction Policy on Downloads/Desktop/TEMP; administrators are exempt).
...
- **Agent** — download `lab-agent.ps1` and install it on each lab PC; it reports heartbeat/status, tracks logins, shows the login gate (Student/Teacher/Visitor/Administrator), scans and quarantines USB devices, watches peripherals, monitors password changes, applies the lab's sign-in method (auto-login on/off) and download/install block policy, and executes queued actions (lock/unlock, WoL relay, remote-view screenshots, and more). Installed agents run at boot as SYSTEM so they cover every user.
...
| GET    | `/api/lab/settings`               | Read lab settings (idle logout, sign-in method, Windows admin account, download/install block) |
